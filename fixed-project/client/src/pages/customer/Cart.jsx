...
const Cart = () => {
  ...
  const fetchCart = async () => {
    try {
      ...
      setCartItems(items);
      setCartCount(items.reduce((acc, item) => acc + (parseInt(item.quantity) || 0), 0));
    } catch (error) {
  ...
  const removeItem = async (itemId) => {
    try {
      await axios.delete(`http://localhost:6001/api/cart/remove-item/${itemId}`);
      fetchCart();
    } catch (error) {
  ...
  const placeOrder = async () => {
    ...
    try {
      await axios.post('http://localhost:6001/api/orders/place-cart-order', {
        name, mobile, email, address, pincode, paymentMethod, orderDate: new Date()
      });
      alert('Order placed!');
  ...
  return (
    <div className="cartPage">
      ...
      <div className="cartPriceBody">
        ...
        <button data-bs-toggle="modal" data-bs-target="#cartCheckoutModal">Place order</button>
      </div>

      <div className="modal fade" id="cartCheckoutModal" data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-hidden="true">
            ...
            <div className="modal-body">
              <div className="checkout-address">
                ...
                <div className="form-floating mb-3">
                  <input type="text" className="form-control" id="c1" value={name} onChange={(e) => setName(e.target.value)} />
                  <label htmlFor="c1">Name</label>
                </div>
                <section>
                  <div className="form-floating mb-3">
                    <input type="text" className="form-control" id="c2" value={mobile} onChange={(e) => setMobile(e.target.value)} />
                    <label htmlFor="c2">Mobile</label>
                  </div>
                  <div className="form-floating mb-3">
                    <input type="text" className="form-control" id="c3" value={email} onChange={(e) => setEmail(e.target.value)} />
                    <label htmlFor="c3">Email</label>
                  </div>
                </section>
                <section>
                  <div className="form-floating mb-3">
                    <input type="text" className="form-control" id="c4" value={address} onChange={(e) => setAddress(e.target.value)} />
                    <label htmlFor="c4">Address</label>
                  </div>
                  <div className="form-floating mb-3">
                    <input type="text" className="form-control" id="c5" value={pincode} onChange={(e) => setPincode(e.target.value)} />
                    <label htmlFor="c5">Pincode</label>
                  </div>
                </section>
              </div>
              <div className="checkout-payment-method">
                ...
                <div className="form-floating mb-3">
                  <select className="form-select" id="c6" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}>
                  ...
                  <label htmlFor="c6">Payment method</label>
                </div>
              </div>
            </div>
